...
export default function Project() {
    ...
    const handleTaskPriorityUpdate = useMutation({
        mutationFn: async (data: { taskId: string; newPriority: string }) => {
    ...
    const handleTaskLabelUpdate = useMutation({
        mutationFn: async (data: { taskId: string; newLabelId: string }) => {
            return await requests.task.updateLabel(projectId, {
                id: data.taskId,
                labelId: data.newLabelId,
            });
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['project', projectId, 'tasks'] });
                            ...
                            <div className="col-span-1 flex justify-end">
                                <StickyNote color={project.color} rotation={0}>
                                    <div className="flex flex-row gap-x-3 px-2">
                                        <button title="Üyeler" className="bg-[rgb(var(--div))] border border-[rgb(var(--border))]/6 text-[rgb(var(--text))] shadow-lg rounded-lg px-2 py-1 cursor-pointer transition-all duration-500 hover:scale-105">
